...
trait PostType {
	...
	public function get_labels( string $singular, string $plural, int $is_female = 0, array $defaults = [] ): array {
		...
		$labels = [
			'add_new' => _n( 'Adicionar nova', 'Adicionar novo', $is_female, 'my-app' ),
			/* translators: %s label singular female lower; %s label singular male lower; */
			'new_item' => sprintf( _n( 'Nova %s', 'Novo %s', $is_female, 'my-app' ), $singular_lower ),
			/* translators: %s label plural lower. */
			'view_items' => sprintf( __( 'Ver %s', 'my-app' ), $plural_lower ),
			'not_found_in_trash' => sprintf(
				/* translators: %s label singular lower. */
				_n( 'Nenhuma %s encontrada na Lixeira.', 'Nenhum %s encontrado na Lixeira.', $is_female, 'my-app' ),
				$singular_lower,
			),
			/* translators: %s label plural lower. */
			...
			'insert_into_item' => sprintf(
				/* translators: %s label singular lower. */
				_n( 'Inserir na %s', 'Inserir no %s', $is_female, 'my-app' ),
				$singular_lower,
			),
			'uploaded_to_this_item' => sprintf(
				/* translators: %s label singular lower. */
				_n( 'Carregado para esta %s', 'Carregado para este %s', $is_female, 'my-app' ),
				$singular_lower,
			),
